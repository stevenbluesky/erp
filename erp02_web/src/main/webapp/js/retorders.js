...
$(function(){	
	initTitle();	//403行-428
	initGrid();		//243行-287
	initOrdersDlg();	//289-369
	initOrderdetailGrid();//372-401
	//入库明细窗口
	$('#itemDlg').dialog({
		title:typeText,
		width:300,
...
function getDataGridCfg(){
	...
	return {
		...
		onDblClickRow:function(rowIndex, rowData){
			...
			$('#createrName').html(formatDat(rowData.createrName));
			$('#checkerName').html(formatDat(rowData.checkerName));
			$('#starterName').html(formatDat(rowData.starterName));
			$('#enderName').html(formatDat(rowData.enderName));
...
function getColumns(){
	...
	if(type == 2){
		return [ [ 
				...
				{field:'createrName',title:'退货员',width:100},
...
function doCheck(){
	$.messager.confirm('确认','确认要审核吗?',function(yes){
		if(yes){
			//提交审核
			$.ajax({
				url : 'returnorders_doCheck.action',//请求的url
 ...
 */
function doBack(){
	$.messager.confirm('确认','确认要驳回吗?',function(yes){
		if(yes){
			//提交驳回操作
			$.ajax({
				url : 'returnorders_doBack.action',//请求的url
				data : {id:$('#uuid').html()},//订单的编号
				dataType : 'json',//返回的数据类型: jquery把返回的值转成json数据
				type : 'post',//请求的方式
				success : function(rtn) {//请求成功时的回调函数, rtn就服务端响应回来的数据
...
function doInOutStore(){
	$.messager.confirm('确认','确认要' + typeText + '吗?',function(yes){
		if(yes){
			...
			var url = 'returnorderdetail_doInStore.action';			
			$.ajax({
				url : url,//请求的url
...
function initGrid(){
	...
	switch(oper){
		...
		/*	$('#statediv').show();*/
		break;
		case 'myreturnorders': url="returnorders_myListByPage.action?";
			datagridCfg.toolbar=[];
			if(type == 2){
				btnText="销售订单录入";
			...
			break;
		case 'doCheck': url="returnorders_listByPage.action?t1.state=0&"; break;
		case 'doInStore': url="returnorders_listByPage.action?t1.state=1&"; break;
	}
	url+='t1.type=' + type;
